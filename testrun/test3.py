...
if __name__ == '__main__':
    ...
    def loss_fn(x, y):
        return torch.mean((1 - chatterjeexi(x.detach().numpy().T[0], y.detach().numpy().T[0]).statistic)*100 + x - x.detach())
    ...
    model.fit(dataset, opt="LBFGS", steps=step_num, lamb=0.001, loss_fn=loss_fn)
    model = model.prune()
    model.fit(dataset, opt="LBFGS", steps=step_num, loss_fn=loss_fn)
    ...
    symbolic_formula = utils.ex_round(symbolic_formula[0][0], 3)
